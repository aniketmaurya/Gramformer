...
class GramformerDemo:
    def __init__(self):
        ...
        self.model_map = {"Corrector": 1, "Detector - coming soon": 2}
        self.examples = [
    ...
    def main(self):
        ...
        model_type = st.sidebar.selectbox(
            label="Choose Model", options=list(self.model_map.keys())
        )
        max_candidates = None
        if model_type == "Corrector":
            max_candidates = st.sidebar.number_input(
                label="Max candidates", min_value=1, max_value=10, value=1
            )
        else:
            # NOTE:
        ...
        input_text = st.selectbox(label="Choose an example", options=self.examples)
